...
- the western way of doing this seems to be with pimoroni's inky line of e-ink displays (NOTE see inkplate). they are not as based, and pi-only, and have a python library. they are probably easier to set up.
...
i still dont know how the chinese way works; it's less clear but it seems to be better in some ways.
...
- battery powered: esp32/pico can be programmed to read data from sd/usb drive (maybe can accept any formatting, and can also take care of image conversion (or not, image data might need to be highly regular coming in)) but it's up to the user to prepare the drive with pictures first.
...
** ideas cont.
with one rpi zero, i need to simulate having 2 MCUs instead, or having only one MCU, or one pi and one esp, or any weird future configuration that would be pretty hard to actually work out physically.
*** config toggle
